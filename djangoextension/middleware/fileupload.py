...
class BoxDotNet(object):
    ...
    def setup(self,api_key=None,ticket=None,auth_token={}):
    ...
    #-------------------------------------------------------------------
    def upload_user(self,auth_token,filename,folder_id,share):
	para = {}
	para['api_key'] = self.api_key
	para['auth_token'] = auth_token
	para['folder_id'] = folder_id
	para['share'] = share
	
	msg = self.upload(filename,**para)
        return msg
...
class BOXNET(object):
    ...
    def __init__(self):
        self.mybox = BoxDotNet()
        self.api_key='bm38xexnzj3lli5zbi8rqo0kmxig6fmy'
	self.auth_token = 'dyprzje0c638yvdkx807tc1xehdn97qu'
    def upload_file(self,auth_token,filename,folder_id,share='0'):
	a = self.mybox.upload_user(auth_token,filename,
...
def handle_uploaded_file_BOXNET(request,filename):
    ''' upload to BOXNET '''
    upload = BOXNET()
    try:
        user_profile = request.user.get_profile()
        folder_id  = user_profile.BOXNET_FolderID
        auth_token = 'dyprzje0c638yvdkx807tc1xehdn97qu'
        a = upload.mybox.upload_user(auth_token,filename,folder_id=folder_id,share='0')
    except:
	pass
