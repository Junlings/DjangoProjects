...
from resources import Resource, FormResource, ModelResource
from response import Response, ErrorResponse
from utils import as_tuple, MSIE_USER_AGENT_REGEX
from utils.mediatypes import is_form_media_type, order_by_precedence
...
class ListModelMixin(ModelMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        #query_kwargs = self.get_query_kwargs(request, *args, **kwargs)
        query_dictlist = request.GET.lists()
        
        query_string = {}
        sorting_string = {}
        for item in query_dictlist:
            if 'sort(' in item[0]: # skip the sort key
                field_name = item[0][6:-1]
                sort_key = item[0][5:6]
                sorting_string[field_name] = sort_key
                
            elif item[1][0] != u'*':  #skip the query for all items
                query_string[str(item[0])] = item[1][0]
            """because there is possible duplicate key and different value,
            use only the first value, use dict()method when update to django1.4"""
            
        query_kwargs = self.get_query_kwargs(request, *args, **query_string)        
        #print query_kwargs
        #print sorting_string
        queryset = queryset.filter(**query_kwargs)
        
        # generate ordering string
        
        ordering = ''
        for key,value in sorting_string.items():
            if value == 'id':
                value = 'pk'   # update 'id' field to pk
            if value == " ":
                str_temp = "-" + key #"+" + key
            else:
                str_temp = key #"-" + key
...
#================customrized model mixin========
class CreateModelUserMixin(ModelMixin):
    """
    Behavior to create a `model` instance on POST requests
    """
    def post(self, request, *args, **kwargs):
        ...
        # here add additional user information
        
        new_dict = self.get_instance_data(model, content, *args, **kwargs)
        print request
        new_dict.update({'owner':request.user})
        instance = model(**new_dict)
        instance.save()
