...
class publication(models.Model):
    ...
    def get_html(self,request):
        mymodel = ContentType.objects.get_for_model(type(self))
        return mymodel,'ll'
    ...
    def get_verbose(self,fun):
        print fun
        if fun == 'get_authors_html':
            return 'Author List'

        elif fun == 'get_keywords_html':
    ...
    def get_authors_html(self):
        ...
        for au in Authorship.objects.filter(publication=self).order_by("sequence"):
            
            author  = au.author
            authorshtml += '<a href="%sauthor/%s/"> %s </a> <br> ' % (settings.ROOT_URL,author.id, '['+str(au.sequence)+ '] ' + author.firstname+',' + author.lastname)
        return authorshtml
    
    def get_keywords_html(self):
        kwhtml = ''
        for kw in self.KWS.all():
            
            kwhtml += '<a href="%spublications/keywords/%s/"> %s </a> <br> ' % (settings.ROOT_URL,kw.id,  kw.KW)
        return kwhtml        
...
class rprt(publication):
    ...
    def get_html(self):
        ...
        t1 = get_template('report_details.html')

        return t1.render(Context({'report':self,
                                  'authors':self.AUs.all(),
                                   'KWs':self.KWS.all(),
                                  }))
    class Meta:
