...
@register.simple_tag 
def simple_tag(object,field):
    try:
        return object._meta.get_field(field).verbose_name  # get the verbose name for the field
        
    except:
        res = object.get_verbose(field)
        return res
...
@register.filter(name='doclink')
def doclink(doc):
    if doc != '':
        return '<a href="%smedia/%s/"> %s </a>' % (settings.ROOT_URL,doc,"Full Text")
    else:
